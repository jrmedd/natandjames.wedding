...
const adjustHex = (hex, amount = 0) => {
  ...
  const target = bounded < 0 ? 0 : 255;
  const p = Math.abs(bounded);
  ...
  const adjusted = channels
    .map((channel) => Math.round((target - channel) * p + channel))
...
const StyledSVG = styled.svg`
  width: auto;
  height: 1.5rem;
  --paypal-dark: ${({ $darkColor }) => $darkColor};
  --paypal-light: ${({ $lightColor }) => $lightColor};
  --paypal-deep: ${({ $deepColor }) => $deepColor};
`;

export const PayPal = () => {
  const theme = useTheme();
  const darkColor = theme?.background || "#6f744a";
  const lightColor = adjustHex(darkColor, 0.28);
  const deepColor = adjustHex(darkColor, -0.12);
